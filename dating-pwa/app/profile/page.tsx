...
export default function ProfilePage() {
  ...
  if (!profile) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-[#080512] space-y-4">
        <p className="text-gray-400 font-bold">Please complete setup to access your dating console.</p>
        <button onClick={() => router.push("/setup")} className="px-6 py-3 bg-pink-600 rounded-2xl text-white font-black shadow-lg shadow-pink-600/30">
          Go to Profile Setup
        </button>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-screen bg-[#080512] pb-24 overflow-y-auto text-white">
      
      {/* Sleek Hero Header Card */}
      <div className="relative pt-8 pb-6 bg-gradient-to-b from-pink-950/60 via-purple-950/40 to-[#080512] px-5 border-b border-white/10 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <span className="text-[11px] bg-white/10 text-pink-300 font-extrabold px-3 py-1 rounded-full border border-white/10 flex items-center gap-1.5">
            <Sparkles size={13} className="text-amber-400 animate-spin" /> VIP Member Console
          </span>
          <div className="flex gap-2">
            <button onClick={() => router.push("/profile/edit")} className="p-2 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-blue-400">
              <Edit3 size={16} />
            </button>
            <button onClick={() => router.push("/settings")} className="p-2 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-gray-300">
              <Settings size={16} />
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl sm:text-2xl font-black text-white flex items-center gap-1.5 truncate">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {hasMilestoneBadge && (
                 <span className="flex items-center gap-1 bg-amber-500/20 text-amber-300 text-[11px] font-black px-2.5 py-0.5 rounded-full border border-amber-500/30">
        ...
        <div className="flex gap-3 mt-6">
           <button onClick={() => router.push('/premium')} className="flex-1 flex items-center justify-center gap-2 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 font-black text-xs py-3 rounded-2xl border border-amber-500/40 transition shadow">
             🪙 {coins} Coins Wallet
           </button>
           ...
           <button 
             ...
             className={`flex-[1.4] flex items-center justify-center gap-2 font-black text-xs py-3 rounded-2xl border transition shadow-lg active:scale-95 ${
               isBoosted 
                 ...
                 : "bg-gradient-to-r from-pink-600 to-purple-600 text-white border-pink-400/50 shadow-pink-600/30"
             }`}
           >
        ...
        <div className="flex bg-white/5 border border-white/10 p-1 rounded-2xl">
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 transition-all ${
              activeTab === "vip" ? "bg-pink-600 text-white shadow-lg shadow-pink-600/30" : "text-gray-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 transition-all ${
              activeTab === "details" ? "bg-purple-600 text-white shadow-lg shadow-purple-600/30" : "text-gray-400 hover:text-white"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold flex items-center justify-center gap-1.5 transition-all ${
              activeTab === "insights" ? "bg-emerald-600 text-white shadow-lg shadow-emerald-600/30" : "text-gray-400 hover:text-white"
            }`}
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              {!profile.verified && (
                <button onClick={() => setShowVerifyModal(true)} className="bg-gradient-to-br from-blue-500/20 to-black border border-blue-500/30 rounded-2xl p-4 flex flex-col items-start gap-1.5 hover:bg-blue-500/30 transition text-left shadow-md">
                  ...
                  <div>
                    ...
                    <p className="text-gray-400 text-[10px]">AI selfie Blue Tick</p>
                  </div>
                </button>
              )}
              
              <button onClick={() => setShowReferralModal(true)} className="bg-gradient-to-br from-emerald-500/20 to-black border border-emerald-500/30 rounded-2xl p-4 flex flex-col items-start gap-1.5 hover:bg-emerald-500/30 transition text-left shadow-md">
                <Gift size={22} className="text-emerald-400" />
                <div>
                  <h3 className="text-emerald-300 font-bold text-xs">Invite & Earn</h3>
                  <p className="text-gray-400 text-[10px]">+200 Coins reward</p>
                </div>
              </button>
              
              <button onClick={() => setShowAdModal(true)} className="bg-gradient-to-br from-amber-500/20 to-black border border-amber-500/30 rounded-2xl p-4 flex flex-col items-start gap-1.5 hover:bg-amber-500/30 transition text-left shadow-md">
                <TrendingUp size={22} className="text-amber-400" />
                <div>
                  <h3 className="text-amber-300 font-bold text-xs">Watch Short Ad</h3>
                  <p className="text-gray-400 text-[10px]">+50 Free Coins</p>
                </div>
              </button>
              ...
              <button onClick={() => setShowStudentModal(true)} className="bg-gradient-to-br from-purple-500/20 to-black border border-purple-500/30 rounded-2xl p-4 flex flex-col items-start gap-1.5 hover:bg-purple-500/30 transition text-left shadow-md">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-[10px]">Verify Campus ID</p>
                </div>
              </button>
            </div>
          ...
          <div className="space-y-5 animate-in fade-in duration-300">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-black text-white uppercase tracking-wider">About Me</h3>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5">
                {profile.location && (
                  <span className="bg-white/10 text-white px-3 py-1 rounded-full text-xs font-bold">📍 {profile.location}</span>
                )}
                {profile.intent && (
                  <span className="bg-pink-500/20 text-pink-300 border border-pink-500/30 px-3 py-1 rounded-full text-xs font-bold">👀 {profile.intent}</span>
                )}
                {profile.campus && (
                ...
                {profile.faith && (
                  <span className="bg-white/10 text-white px-3 py-1 rounded-full text-xs font-bold">🙏 {profile.faith}</span>
                )}
                {profile.orientation && (
                  <span className="bg-white/10 text-white px-3 py-1 rounded-full text-xs font-bold">🏳️‍🌈 {profile.orientation}</span>
                )}
              </div>
            </div>
            ...
            {profile.hobbies && profile.hobbies.length > 0 && (
              <div className="bg-white/5 border border-white/10 rounded-3xl p-5 space-y-3">
                <h3 className="text-sm font-black text-white uppercase tracking-wider">My Interests</h3>
            ...
            {profile.prompts && profile.prompts.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-sm font-black text-white uppercase tracking-wider px-1">My Prompts</h3>
                {profile.prompts.map((p, i) => (
                  <div key={i} className="bg-gradient-to-r from-purple-950/40 to-black border border-purple-500/30 rounded-2xl p-4 shadow-md">
                    ...
                    <p className="text-white text-xs font-bold">{p.answer}</p>
                  </div>
                ))}
              </div>
            )}
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-dark-bg border border-white/20 w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl">
            ...
            <h3 className="text-lg font-extrabold text-white mb-2">Free AI Verification</h3>
            <p className="text-xs text-gray-400 mb-6">Take a quick selfie to verify your real identity and earn the trusted Blue Tick on your profile.</p>
            ...
            {isVerifying ? (
              ...
              <button 
                ...
                className="w-full py-3 rounded-2xl bg-blue-600 font-black text-xs text-white shadow-lg shadow-blue-600/30"
              >
                Start Camera Scan
              </button>
            )}
            {!isVerifying && (
              <button onClick={() => setShowVerifyModal(false)} className="w-full mt-3 py-2 text-xs text-gray-400 font-bold">Cancel</button>
            )}
          </div>
        </div>
      )}
      ...
      {showReferralModal && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-dark-bg border border-white/20 w-full max-w-sm rounded-3xl p-6 shadow-2xl">
            <div className="flex justify-between items-start mb-3">
              <div className="w-10 h-10 rounded-2xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center">
                <Gift size={20} />
              </div>
              <button onClick={() => setShowReferralModal(false)} className="text-gray-400 hover:text-white"><X size={20} /></button>
            </div>
            <h3 className="text-base font-black text-white mb-1">Invite & Earn 200 Coins</h3>
            <p className="text-xs text-gray-400 mb-5">Share your invite code with friends. You both receive <span className="text-amber-400 font-bold">200 Free Coins</span> when they join LoveWithYou!</p>
            
            <div className="flex items-center gap-2 p-3 bg-white/5 border border-white/10 rounded-2xl mb-5 text-xs">
              <span className="flex-1 font-mono text-gray-300 truncate">lovewithyou.app/invite/{profile.name.toLowerCase()}</span>
              <button 
                onClick={() => { navigator.clipboard.writeText(`https://lovewithyou.app/invite/${profile.name.toLowerCase()}`); toast("Link Copied!", "success"); }}
                className="p-2 bg-white/10 hover:bg-white/20 rounded-xl text-white font-bold"
              >
                <Copy size={14} />
              </button>
            </div>
            
            <button onClick={() => setShowReferralModal(false)} className="w-full py-3 rounded-2xl bg-emerald-600 text-white font-extrabold text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/30">
      ...
      {showAdModal && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-dark-bg border border-white/20 w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl">
             ...
             <h3 className="text-base font-black text-white mb-1">Watch Short Video Ad</h3>
             <p className="text-xs text-gray-400 mb-5">Support LoveWithYou and receive <span className="text-amber-300 font-bold">+50 Coins</span> instantly for your wallet!</p>
             
             {isWatchingAd ? (
               <div className="w-full h-36 bg-black rounded-2xl border border-white/10 flex flex-col items-center justify-center mb-4 relative overflow-hidden">
                 ...
                 <p className="text-white font-extrabold text-xs">Playing Sponsor Video...</p>
                 <p className="text-gray-500 text-[10px] mt-1">Please wait 3 seconds...</p>
               </div>
             ) : (
               <button 
                 onClick={() => {
                   ...
                   setTimeout(() => {
                     ...
                     toast("Earned +50 Coins for your wallet! 🪙", "success");
                   }, 3000);
                 }}
             ...
             {!isWatchingAd && (
               <button onClick={() => setShowAdModal(false)} className="w-full mt-2.5 py-2 text-xs text-gray-400 font-bold">Cancel</button>
             )}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-dark-bg border border-white/20 w-full max-w-sm rounded-3xl p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-base font-black text-white flex items-center gap-2"><MessageSquare size={16} /> Send Us Feedback</h3>
              <button onClick={() => setShowFeedbackModal(false)} className="text-gray-400 hover:text-white"><X size={18} /></button>
            </div>
            <textarea 
              rows={4}
              value={feedbackText}
              ...
              className="w-full bg-black/60 border border-white/20 rounded-2xl p-3 text-xs text-white outline-none focus:border-pink-500 resize-none mb-4 font-medium shadow-inner"
            />
            <button 
              onClick={async () => {
              ...
              className="w-full py-3 rounded-2xl bg-pink-600 text-white font-black text-xs shadow-lg shadow-pink-600/30"
            >
              Submit Feedback
            </button>
          </div>
