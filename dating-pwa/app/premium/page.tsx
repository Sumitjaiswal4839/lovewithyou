...
export default function PremiumPage() {
  ...
  const TIERS = [
    ...
    },
    {
      ...
      fullPrice: "₹799/mo",
      studentPrice: "₹399/mo",
      color: "bg-gradient-to-r from-primary to-purple-600 text-foreground",
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground overflow-y-auto font-sans pb-24">
      {/* Header */}
      <div className="p-4 pt-6 flex items-center justify-between sticky top-0 bg-background/90 backdrop-blur-xl border-b border-border z-10">
        <div className="flex items-center gap-3">
          <button onClick={() => router.back()} className="p-2 -ml-2 rounded-xl hover:bg-surface-elevated text-foreground transition">
            <ArrowLeft size={22} />
          </button>
          <div>
            <h2 className="text-xl font-black text-foreground flex items-center gap-2 tracking-tight">
              VIP &amp; Coin Marketplace <Crown size={20} className="text-warning animate-pulse" />
            </h2>
            <p className="text-[11px] text-muted font-medium">Dynamic perks, cashback &amp; verified filters</p>
          </div>
        </div>

        {/* Dynamic Student Status Badge */}
        {isStudent && (
          <span className="text-[10px] bg-success/20 text-emerald-300 font-extrabold px-3 py-1.5 rounded-full border border-emerald-500/40 flex items-center gap-1.5 shadow-sm">
            <GraduationCap size={14} className="text-success" /> 50% STUDENT DISCOUNT
          </span>
        )}
      </div>
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-sm font-black text-foreground uppercase tracking-wider flex items-center gap-2">
              <Coins size={18} className="text-warning" /> Buy Coins (Razorpay UPI / Cards)
            </h3>
            <span className="text-xs text-amber-300 font-bold">
              Balance: <span className="font-black text-foreground">{coins} 🪙</span>
            </span>
          </div>
            ...
            {[
              ...
              return (
                ...
                >
                  ...
                  <h4 className="font-bold text-xs text-foreground">{pack.name}</h4>
                  <div className="text-base font-black text-success my-2">
                    ₹{finalP} {isStudent && <span className="text-[10px] text-muted line-through font-normal">₹{pack.price}</span>}
                  </div>
                  <button
                    onClick={() => handleRazorpayBuyCoins(pack.amount, pack.price, pack.name)}
                    className="w-full py-2.5 rounded-2xl bg-gradient-to-r from-amber-500 to-primary hover:from-amber-600 hover:to-rose-600 font-black text-xs text-black transition shadow-md active:scale-95 flex items-center justify-center gap-1"
                  >
                    Buy via Razorpay
                  </button>
                </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            <h3 className="text-sm font-black text-foreground uppercase tracking-wider flex items-center gap-2">
              <Crown size={16} className="text-warning" /> Subscription Tiers
            </h3>
            {isStudent && <span className="text-[10px] text-success font-bold">50% Student Pricing Applied</span>}
          </div>
          ...
          <div className="flex overflow-x-auto gap-4 snap-x snap-mandatory hide-scrollbar pb-2">
            {TIERS.map(tier => (
              <div
                ...
                className={`min-w-[270px] snap-center rounded-3xl p-5 border-2 transition-all cursor-pointer relative bg-white/[0.03] ${selectedTier === tier.name ? `border-primary ${tier.shadow}` : 'border-border opacity-80 scale-98'}`}
              >
                {isStudent && (
                  <span className="absolute top-4 right-4 text-[9px] bg-success/20 text-emerald-300 font-black px-2 py-0.5 rounded-full border border-emerald-500/30 uppercase">
                    50% Off Student
                  </span>
                )}
                ...
                <div className="flex items-baseline gap-2 mb-4">
                  <span className="text-2xl font-black text-foreground">{isStudent ? tier.studentPrice : tier.fullPrice}</span>
                  {isStudent && <span className="text-xs text-muted line-through font-bold">{tier.fullPrice}</span>}
                </div>

                <ul className="space-y-2.5 mb-2">
                  {tier.features.map(f => (
                    <li key={f} className="flex items-center gap-2 text-xs text-secondary font-medium">
          ...
          <Button 
            ...
            className="w-full mt-3 text-sm py-4 rounded-2xl font-black bg-gradient-to-r from-primary via-pink-600 to-purple-600 text-white shadow-lg shadow-primary/25 active:scale-95 transition"
          >
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-1">
            <h3 className="text-sm font-black text-foreground uppercase tracking-wider flex items-center gap-2">
              <Coins size={16} className="text-warning" /> Coin Perks Marketplace
            </h3>
            <span className="text-xs text-amber-300 font-bold flex items-center gap-1">
              Wallet: <span className="font-black text-foreground">{coins} 🪙</span>
            </span>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/[0.03] border border-border hover:border-primary/40 rounded-3xl p-4 text-center transition shadow-md">
              <Sparkles size={24} className="text-primary mx-auto mb-2 animate-pulse" />
              <h4 className="font-black text-sm text-foreground">5 Super Likes</h4>
              <p className="text-[10px] text-muted mt-0.5">Stand out in swipe decks</p>
              <button 
                onClick={() => handleRedeemPerk(25, "5 Super Likes")}
                className="w-full mt-3 py-2 rounded-xl bg-surface-elevated hover:bg-surface-elevated text-xs font-black text-amber-300 border border-border"
              >
                Trade 25 🪙
              </button>
            </div>

            <div className="bg-white/[0.03] border border-border hover:border-purple-500/40 rounded-3xl p-4 text-center transition shadow-md">
              <Zap size={24} className="text-purple-400 mx-auto mb-2 animate-pulse" />
              <h4 className="font-black text-sm text-foreground">Radar Boost</h4>
              <p className="text-[10px] text-muted mt-0.5">Top profile for 30 mins</p>
              <button 
            ...
            <div className="bg-white/[0.03] border border-border hover:border-cyan-500/40 rounded-3xl p-4 text-center transition shadow-md">
              <Shield size={24} className="text-cyan-400 mx-auto mb-2" />
              <h4 className="font-black text-sm text-foreground">Blue Diamond Badge</h4>
              <p className="text-[10px] text-muted mt-0.5">Trust Shield Pass</p>
              <button 
            ...
            <div className="bg-white/[0.03] border border-border hover:border-emerald-500/40 rounded-3xl p-4 text-center transition shadow-md">
              <EyeOff size={24} className="text-success mx-auto mb-2" />
              <h4 className="font-black text-sm text-foreground">24H Ad-Free Pass</h4>
              <p className="text-[10px] text-muted mt-0.5">Silence all ads</p>
              <button 
                onClick={() => {
                  if (coins >= 80) {
                    spendCoins(80);
        ...
        <div className="bg-white/[0.03] border border-border rounded-3xl p-5 flex items-center justify-between shadow-md">
          <div className="space-y-1">
            <h4 className="text-sm font-black text-foreground flex items-center gap-2">
              🚫 Ad-Free Experience Tier
            </h4>
            <p className="text-xs text-muted">Silence video ad popups and banner ads across all screens</p>
          </div>
          <button
            onClick={() => {
              toggleAdFree();
            ...
            className={`px-4 py-2 rounded-2xl text-xs font-black transition border ${
              adFreeEnabled
                ? "bg-success/20 text-emerald-300 border-emerald-500/40"
        ...
        <div className="bg-white/[0.03] border border-border rounded-3xl p-5 space-y-4 shadow-md">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-black text-foreground flex items-center gap-2">
              <Filter size={16} className="text-primary" /> Premium Matching Filters
            </h4>
            <span className="text-[10px] text-primary bg-primary/10 border border-primary/20 px-2 py-0.5 rounded-full font-bold">VIP Feature</span>
          </div>
          ...
          <div className="space-y-2.5">
            <button
              ...
              className={`w-full p-3 rounded-2xl flex items-center justify-between text-xs font-bold transition border ${
                matchPreferences.verifiedOnly ? "bg-primary/15 border-primary/40 text-primary" : "bg-surface-elevated border-border text-white/60"
              }`}
            >
              <span className="flex items-center gap-2">🛡️ Show Verified Blue Diamond Profiles Only</span>
              {matchPreferences.verifiedOnly && <Check size={16} className="text-primary" />}
            </button>
            ...
            <button
              ...
              className={`w-full p-3 rounded-2xl flex items-center justify-between text-xs font-bold transition border ${
                matchPreferences.studentsOnly ? "bg-success/15 border-emerald-500/40 text-emerald-300" : "bg-surface-elevated border-border text-muted"
              }`}
            >
              <span className="flex items-center gap-2">🎓 Show Verified College Students Only</span>
              {matchPreferences.studentsOnly && <Check size={16} className="text-success" />}
            </button>
            ...
            <button
              ...
              className={`w-full p-3 rounded-2xl flex items-center justify-between text-xs font-bold transition border ${
                matchPreferences.zodiacMatchOnly ? "bg-purple-500/15 border-purple-500/40 text-purple-300" : "bg-surface-elevated border-border text-muted"
              }`}
            >
        ...
        {!adFreeEnabled && (
          <div
            ...
            className="bg-white/[0.03] border border-border hover:border-amber-500/30 rounded-3xl p-4 flex items-center justify-between cursor-pointer transition shadow-md"
          >
            <div>
              <h4 className="font-black text-sm text-foreground flex items-center gap-2">
                <PlayCircle size={18} className="text-warning" /> Watch Sponsored Ad
              </h4>
              <p className="text-xs text-muted mt-0.5">Watch a 15s video to claim 5 Free Coins</p>
            </div>
      ...
      {showAdsModal && (
        <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex flex-col justify-center items-center p-4">
          <div className="w-full max-w-sm aspect-video bg-black rounded-3xl flex items-center justify-center border border-white/20 mb-6 shadow-2xl relative overflow-hidden">
            <span className="text-primary font-bold text-xs animate-pulse">Playing Rewarded Video Ad... (15s)</span>
          </div>
          <button
            onClick={() => {
              addCoins(20, "watch_ad");
              toast("🎉 You claimed 20 Free Coins + 10% Cashback Bonus!", "success");
              setShowAdsModal(false);
            }}
            className="px-8 py-3 bg-gradient-to-r from-primary to-pink-600 rounded-2xl font-black text-xs text-white shadow-lg shadow-primary/30"
          >
