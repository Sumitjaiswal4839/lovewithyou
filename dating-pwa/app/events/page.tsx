...
export default function EventsPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-background p-6 pt-12 pb-20">
      <div className="flex items-center gap-3 mb-6">
         ...
         <div>
            ...
            <p className="text-sm text-muted">Join exclusive themed matching events.</p>
         </div>
      </div>

      <div className="space-y-4">
        {events.map(ev => (
          <div key={ev.id} className="bg-surface-elevated border border-border rounded-2xl p-4 hover:bg-surface-elevated transition cursor-pointer relative overflow-hidden">
             <div className="absolute top-0 right-0 w-24 h-24 bg-violet-500/10 rounded-bl-full -mr-4 -mt-4 blur-xl"></div>
             <h3 className="text-foreground font-bold text-lg">{ev.title}</h3>
             ...
             <div className="mt-4 flex items-center justify-between">
                <span className="text-xs font-medium text-foreground/60">{ev.participants} attending</span>
                <button className="px-4 py-1.5 bg-violet-500 hover:bg-violet-600 text-foreground text-xs font-bold rounded-full transition">
                  RSVP
                </button>
             </div>
          </div>
